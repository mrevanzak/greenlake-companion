...
class PDFBuilder {
  ...
  private func drawSingleTask(task: LandscapingTask, in frame: CGRect) {
    ...
    let titleHeight = attributedTitle.boundingRect(
      with: CGSize(width: contentWidth, height: .infinity), options: .usesLineFragmentOrigin,
      context: nil
    ).height
    ...
    let attributedDescription = NSAttributedString(
      ...
      attributes: [.font: descriptionFont, .foregroundColor: UIColor.darkGray])
    let descriptionHeight = attributedDescription.boundingRect(
      with: CGSize(width: contentWidth, height: .infinity), options: .usesLineFragmentOrigin,
      context: nil
    ).height
    let descriptionRect = CGRect(
      x: contentX, y: contentY, width: contentWidth, height: descriptionHeight)
    ...
    for detail in details {
      ...
      let valueRect = CGRect(
        x: frame.maxX - internalPadding - valueSize.width, y: contentY, width: valueSize.width,
        height: lineHeight)
  ...
  private func calculateTaskHeight(task: LandscapingTask, width: CGFloat) -> CGFloat {
    ...
    let valueText = NSAttributedString(
      string: "Test", attributes: [.font: detailsFont, .foregroundColor: UIColor.black])
