...
struct PDFSampleView: View {
  ...
  var body: some View {
    ...
    .padding()
    // When pdfPreview is set, this sheet will appear
    .sheet(item: $pdfPreview) { pdfDataWrapper in
      // Add a top bar with a done button for dismissal
      NavigationView {
  ...
  private func generatePDF() {
    // Make sure your images are in the asset catalog!
    guard let img1 = UIImage(named: "img1"),
          let img2 = UIImage(named: "img2"),
          let img3 = UIImage(named: "img3"),
          let img4 = UIImage(named: "img4") else {
      print("⚠️ Error: Could not load one or more images from assets. Make sure 'img1' through 'img4' exist.")
      return
    }
    ...
    let pdfData = creator.createPDF { pdf in
      // The `build` closure lets you add components sequentially
      pdf.drawTitle(text: reportTitle)
      pdf.drawParagraph(text: "This document outlines the key achievements and milestones from the past quarter. We have seen significant growth in user engagement and market penetration.")
      ...
      pdf.drawParagraph(text: "A selection of images from recent company events and product launches. The following images are aligned to the center as a group.")
